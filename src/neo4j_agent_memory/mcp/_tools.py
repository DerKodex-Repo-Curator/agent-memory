...
def _register_core_tools(mcp: FastMCP) -> None:
    ...
    async def memory_add_fact(
        ...
        # Link subject and object entities to the fact so they are discoverable
        fact_id = result.get("id") if isinstance(result, dict) else None
        if fact_id:
            linked_entities = []
            name = await _ensure_entity_linked(
                client, subject, "PERSON", fact_id, "fact", role="subject"
            )
            if name:
                linked_entities.append(name)
            name = await _ensure_entity_linked(
                client, object_value, "OBJECT", fact_id, "fact", role="object"
            )
            if name:
                linked_entities.append(name)
            if linked_entities and isinstance(result, dict):
                result = dict(result)
